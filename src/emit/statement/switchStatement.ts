...
export const switchStatementEmitter: Emitter<ts.SwitchStatement> = (node, option) => {
  ...
  const clauseEmitters = node.caseBlock.clauses.map((clause): ClauseEmitter => {
    if (ts.isCaseClause(clause)) {
      ...
      const statementsEmitters = clause.statements.map((stmt) => getEmitNode(stmt, option));
  ...
  return {
    emit: () => {
      ...
      const clausesCode = clauseEmitters.map((clause) => {
        if (clause.type === "case") {
          const caseExpression = clause.expression.emit();
          const statementsCode = clause.statements.map((stmt) => stmt.emit()).join("\n    ");
    ...
    getAllVars: () => {
      ...
      const clausesVarsSets = clauseEmitters.map((clause) => {
        if (clause.type === "case") {
          ...
          const statementsVars = clause.statements.map(stmt => stmt.getAllVars());
